...
domain: D1
topics: ["core components of the Databricks Data Intelligence Platform", "architecture, Delta Lake, and Unity Catalog fundamentals"]
sources: ["sources/exam-guide.md"]
status: drafted
...
```mermaid
flowchart TD
    V["Vault — coldwater<br/>(one authority over many ledgers)"]
    L1["Ledger — marchlands"]
    L2["Ledger — hollowmere"]
    F1["Leaf — tolls"]
    F2["Leaf — tolls"]
    V --> L1
    V --> L2
    L1 --> F1
    L2 --> F2
    F1 -.->|"same short name<br/>different record"| F2
```
